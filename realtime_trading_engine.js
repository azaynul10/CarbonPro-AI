...
export class RealTimeTradingEngine {
   ...
   */
  async attemptOrderMatching(newOrder) {
    try {
   ...
   */
  async updateOrderQuantities(buyOrder, sellOrder, executedQuantity) {
    const updates = [];
   ...
   */
  async updateMarketData(transaction) {
    try {
      const now = new Date();
...
class OrderBook {
  ...
  addOrder(order) {
    ...
    orderMap.get(price).sort((a, b) => new Date(a.created_at) - new Date(b.created_at));
  }
...
class PriceEngine {
  ...
  getVWAP(creditId, timeWindow = 3600000) { // 1 hour default
    const now = new Date();
 ...
 */
export function useRealTimeTrading(supabaseClient) {
  ...
  useEffect(() => {
    ...
    const unsubscribeTrades = engine.subscribe('trade_executed', (trade) => {
      setRecentTrades(prev => [trade, ...prev.slice(0, 99)]); // Keep last 100 trades
    });
    const unsubscribeMarketData = engine.subscribe('market_data_updated', (data) => {
  ...
  const cancelOrder = useCallback(async (orderId, userId) => {
    if (!tradingEngine) return { success: false, error: 'Trading engine not initialized' };
